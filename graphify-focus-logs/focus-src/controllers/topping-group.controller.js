import * as repo from '../storage/repositories/topping-group.repo.js';
...
export async function createGroup(req, res) {
  try {
    const { name, required = false, sort_order = 0 } = req.body;
    if (!name) {
      return res.status(400).json({ error: 'Missing required fields: name' });
    }
    const result = repo.create({ name, required, sort_order });
