 ...
 * @param {string|undefined} targetDate - Optional YYYY-MM-DD date. Defaults to today's store date.
 * @returns {{todayStoreDate: string, shiftsForDate: Array<object>}}
 */
function _getDayCloseContext(targetDate) {
  ...
  return { todayStoreDate: storeDate, shiftsForDate };
}
...
export async function getReviewDataForDayClose(targetDate) {
  const { todayStoreDate, shiftsForDate } = _getDayCloseContext(targetDate);
